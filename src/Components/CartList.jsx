...
function CartList() {
  const { listedProducts, setListedPrdcts, cartListActive, setCartListActive } = useContext(productContext);
  ...
  const grandTotal = listedProducts?.reduce(
    (total, product) => total + (product.productPrice || 0) * (product.quantity || 1),
    0
  ) || 0;
...
<section
  ...
  className={
    `fixed top-0 right-0 h-screen z-30 flex flex-col justify-between bg-gray-800 transition-all duration-300 ease-in-out ${cartListActive 
